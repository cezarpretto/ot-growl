module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		cssmin: {
			core: {
				files: {
					'build/ot-growl.min.css': '<%= lib_files.css %>'
				},
				options: {
					'banner': '<%= meta.banner %>',
					'report': 'gzip'
		...
		uglify: {
			core: {
				files: {
					'<%= build_dir %>/ot-growl.min.js': '<%= concat.core.dest %>'
				},
				options: {
					banner: '<%= meta.banner %>',
					report: 'gzip'
		...
		push: {
			options: {
				...
				commitFiles: ['package.json', 'bower.json', 'build/ot-growl.js', 'build/ot-growl.min.js', 'build/ot-growl.min.css', 'README.md'], // '-a' for all files
				createTag: true,
				tagName: 'v%VERSION%',
